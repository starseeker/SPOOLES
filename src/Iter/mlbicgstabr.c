...
/*
   ...
   using right preconditioned ML(k)BiCGSTAB method 
...
/*
   --------------------
   init the vectors in MLBiCGSTAB
   --------------------
*/
mtxD = DenseMtx_new() ;
...
error_trol = Init_norm * convergetol ;
  ...
  fprintf(msgFile, "\n MLBiCGSTAB Initial norml: %6.2e ", Init_norm ) ;
  fprintf(msgFile, "\n MLBiCGSTAB Conveg. Control: %7.3e ", convergetol ) ;
  fprintf(msgFile, "\n MLBiCGSTAB Convergen Control: %7.3e ",error_trol ) ;
...
while (  Iter <= itermax ){
    ...
    Iter++;
/*
    DenseMtx_colCopy (vecT,0, mtxG,Ik-1);
...
/*
    ...
    FrontMtx_solve(Precond, vecT1, vecT, Precond->manager,
		   cpus, msglvl, msgFile) ;
...
fprintf(msgFile, "\n MLBiCGSTAB True Residual norm: %6.2e ", 
	Rtmp) ;
fprintf(msgFile, "\n\n after MLBiCGSTABR") ;
DenseMtx_free(mtxD) ;
DenseMtx_free(mtxG) ;
